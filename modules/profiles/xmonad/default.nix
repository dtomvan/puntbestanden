let
  # TODO: turn off
  enableDev = true;
  zellijDevLayout = builtins.toFile "xmonad-dev.kdl" ''
    layout {
      pane size=1 borderless=true {
          plugin location="tab-bar"
      }
      pane split_direction="vertical" {
        pane edit="./modules/profiles/xmonad/default.nix"
        pane
      }
      pane size=1 borderless=true {
        ...
        pkgs.writers.writeHaskellBin "xmonad-${system}"
          ...
          ''
            ...
            myStartupHook = do
              ...
              ${lib.optionalString enableDev ''spawnOnce "EDITOR=nvim alacritty --working-directory ~/puntbestanden -e zellij -n ${zellijDevLayout}"''}
  ...
  flake.modules.nixos.profiles-xmonad =
    ...
    in
    {
      ...
      services.xserver.windowManager.session = lib.singleton {
        name = "xmonad";
        start = ''
          if which ${xmonadName} && ${toString enableDev}; then
            ${xmonadName}
          else
            systemd-cat -t xmonad -- ${lib.getExe' myPkgs.myXmonad xmonadName}
          fi
          waitPID=$!
        '';
      };
