---
title: "Re: We made AI, stop complaining"
summary: |
  I don't think it's fair to say "we" have made AI. Researchers and VC funds and (wannabe) billionaires did.
show-summary: false
date: 2026-07-11T17:23:53+02:00
tags:
  - clankers
  - thoughts
  - JulyReply
layout: post
lang: en
draft: false
---

In-reply-to: https://www.gordonmclean.co.uk/2026/07/11/we-made-ai-stop-complaining/

Please read the above post first. It's really bite-sized. This started as a comment on Gordon's blog but I decided I'd post it here as well (if only for JulyReply 😆).

---

I don't think it's fair to say "we" have made AI. Researchers and VC funds and (wannabe) billionaires did. Except for the people tho are way to gullible to reject these technologies, who can actually push back on it? I mean we do live in a society yada yada and we collectively started voting with our wallets and data so the companies continued doing their shitty practices, but really that "voting" in my experience mostly stems from ignorance, and the lack of thinking critically about "does this technology exist to serve me, or to serve the billionaires?"

I for one was really really surprised to find out how many people around me started relying of ChatGPT, and how nobody really stopped to think about how a handful of years ago nobody even knew what an LLM was...

---

Tagged #clankers, because I hate 'em and I think a slur is justified here. I really think we as a society shouldn't rely on these tools as much as we are doing right now [^1] [^2]

Thank you for listening to my rant. Happy [JulyReply](https://robertbirming.com/julyreply-2026-blog-connecting/).

![julyreply-2026](/assets/img/julyreply-2026.svg)

[^1]: re: [AI 'Therapist' told me to kill myself and others -- Caelan Conrad](https://www.youtube.com/watch?v=lfEJ4DbjZYg); it's a YouTube video, apologies, but I think a really important one
[^2]: also, this is kind-of hypocritical, since I fall back on an LLM if I can't figure out something nontrivial to point me in the right direction, every now and again (lately: nginx configurations 😂)
