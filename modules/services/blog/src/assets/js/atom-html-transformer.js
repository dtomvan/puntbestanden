...
var head = h("head", {}, h("style", {}, `feed > * { display: none; }`));
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var newLink = h(
    "h1",
    {},
    h("a", { href: feedLink.getAttribute("href") }, ...feedTitle.childNodes),
  );
  feedTitle.replaceChildren(newLink);
  newLink.insertAdjacentHTML(
    "afterend",
    "<p>This is an <b>atom</b> feed. Add it to your reader, or just read by clicking links from here!</p>",
  );
  ...
  document.querySelectorAll("entry").forEach((entry) => {
    ...
    entry.querySelectorAll("published, updated").forEach((date) => {
      ...
      var i = h("i");
      i.innerHTML =
        (date.tagName == "published" ? "Published " : "Last updated ") + newDate.outerHTML;
    ...
    entry.querySelectorAll("category").forEach((category) => {
      ...
      var tag = h(
        "a",
        {
          ...
          style: "color: var(--ctp-subtext0);text-decoration: none;",
        },
        tagSpan,
      );
    ...
    if (repliesLink) {
      entry.insertAdjacentElement(
        ...
        h(
          "p",
          { class: "replies-link" },
          h("a", { href: repliesLink.getAttribute("href") }, repliesLink.getAttribute("title")),
        ),
      );
    }
  });
