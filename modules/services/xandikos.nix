...
{ lib, ... }:
{
  flake.modules.nixos.services-xandikos =
    { config, modulesPath, ... }:
    {
      imports = [
        {
          disabledModules = [ "${modulesPath}/services/networking/xandikos.nix" ];
          sops.secrets.xandikos = {
            sopsFile = ../../secrets/xandikos.secret;
            format = "binary";
            owner = "nginx";
            group = "nginx";
            mode = "0440";
          ...
          services.xandikos = {
            ...
            nginx = {
              ...
              extraConfig = ''
                auth_basic "Xandikos";
                auth_basic_user_file ${config.sops.secrets.xandikos.path};
              '';
            };
          };
        }
                ...
                nginx = mkOption {
                  type = submodule {
                    imports = [
                      ...
                      (mkRemovedOptionModule [ "enable" ] "")
                      (mkRemovedOptionModule [ "hostName" ] "")
                    ];
                  };
                  default = { };
                  description = ''
